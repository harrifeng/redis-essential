...
function sendDealIfNotSent(dealId, userId) {
  client.sismember(dealId, userId, function(err, reply) {
    if (reply) {
      console.log("Deal", dealId, "was already sent ot user", userId)
    } else {
...
// <===================OUTPUT===================>
// Deal deal:1 was already sent ot user user:1
// Deal deal:1 was already sent ot user user:2
// Sending deal:1 to user user:3
// user:1 received all of the deals: deal:1,deal:2
// user:3,user:2,user:1 received at least one of the deals: deal:1,deal:2
